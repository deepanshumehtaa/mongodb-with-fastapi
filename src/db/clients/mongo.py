...
async def acquire_id(db: DB, seq_name: str, count: int = 1) -> range:
    """ Used to create auto-incrementing IDs """
...
class RWModel(BaseModel):
    ...
    async def save(self: T, db: DB, fields: Iterable[str] = None) -> T:
        ...
        Note
        ----
        This step handles three special fields: `id`, `created_time`, and `updated_time`.

        During storage, it checks `self.id`:

        - None:
        Creates a new record, with `created_time` and `updated_time` set to the values ​​of `self`, forcing an update to the database.

        - int:
        Updates the record. At this point, it checks the `fields` parameter. If

        - "created_time" and "updated_time" are passed in, the corresponding fields in `self` are updated to the db.
        - Otherwise, `updated_time` is automatically updated to `now`, and `created_time` is not updated.
    ...
    async def batch_create(
        ...
        When calling this method externally, there is no need to consider batch creation.
        The batch creation size is specified by the `batch_size` parameter.
        """
        if not objects:
            return []
    ...
    async def batch_update(
        ...
        if not valid_fields:
            raise ValueError(f"Incorrect parameters were passed during batch update.：fields：{fields}")
