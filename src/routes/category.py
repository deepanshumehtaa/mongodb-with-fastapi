...
@cat_router.put("/category/{cat_id}",
                tags=["category"],
                dependencies=[Depends(auth.login_required)])
...
@cat_router.delete("/category/{cat_id}",
                   tags=["category"],
                   dependencies=[Depends(auth.login_required)])
async def delete_category(cat_id: int = Path(..., embed=True),
                          is_expenditure: bool = Body(..., embed=True),
                          resource: CategoryResource = Depends(CategoryResource)
) -> BaseResponse:
...
@cat_router.post("/category/sort",
                 tags=["category"],
                 dependencies=[Depends(auth.login_required)])
async def sort_category(cat_id_list: List[int] = Body(..., embed=True),
                        is_expenditure: bool = Body(..., embed=True),
                        resource: CategoryResource = Depends(CategoryResource),
) -> BaseResponse:
